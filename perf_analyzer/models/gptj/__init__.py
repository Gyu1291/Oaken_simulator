...
class GPTJForCausalLM(NodeMixin):
  # Constructor
  def __init__(self, config: GPTJConfig):
    super(GPTJForCausalLM, self).__init__()
    # Config
    self.config = config
    ...
    self.layers = list(
      [GPTJDecoderLayer(config, layer_idx, parent=self) for layer_idx in range(config.num_hidden_layers)]
    )
    self.ln_f = LayerNorm(config.hidden_size, parent=self)
    self.lm_head = LMHead(config.hidden_size, config.vocab_size, use_table=False, use_bias=True, parent=self)
...
class GPTJDecoderLayer(NodeMixin):
  # Constructor
  def __init__(self, config: GPTJConfig, layer_idx: int, name=None, parent=None):
    # Config
    self.config = config
