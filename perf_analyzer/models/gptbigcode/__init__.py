...
class GPTBigCodeConfig(BaseConfig):
...
class GPTBigCodeForCausalLM(NodeMixin):
  # Constructor
  def __init__(self, config: GPTBigCodeConfig):
    super(GPTBigCodeForCausalLM, self).__init__()
    # Config
    self.config = config
    self.name = self.__class__.__name__
    # Construct model
    self.tok_embed = TokenEmbedding(config.hidden_size, config.vocab_size, parent=self)
    self.pos_embed = PositionalEmbedding(config.hidden_size, config.max_length, parent=self)
    self.layers = list(
      [GPTBigCodeDecoderLayer(config, layer_idx, parent=self) for layer_idx in range(config.num_hidden_layers)]
    )
  ...
  def n_op(self):
    ...
    op = op + self.pos_embed.n_op()       # Positional embedding
  ...
  def n_param(self):
    ...
    size = size + self.pos_embed.n_param()
    for layer in self.layers:
...
class GPTBigCodeDecoderLayer(NodeMixin):
  # Constructor
  def __init__(self, config: GPTBigCodeConfig, layer_idx: int, name=None, parent=None):
    super(GPTBigCodeDecoderLayer, self).__init__()
    # Config
    self.config = config
