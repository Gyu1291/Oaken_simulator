...
class OrionForCausalLM(NodeMixin):
  # Constructor
  def __init__(self, config: OrionConfig):
    super(OrionForCausalLM, self).__init__()
    # Config
    self.config = config
    ...
    self.layers = list(
      [OrionDecoderLayer(config, layer_idx, parent=self) for layer_idx in range(config.num_hidden_layers)]
    )
...
class OrionDecoderLayer(NodeMixin):
  # Constructor
  def __init__(self, config: OrionConfig, layer_idx: int, name=None, parent=None):
    super(OrionDecoderLayer, self).__init__()
    # Config
    self.config = config
