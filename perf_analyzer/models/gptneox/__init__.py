...
class GPTNeoXForCausalLM(NodeMixin):
  # Constructor
  def __init__(self, config: GPTNeoXConfig):
    super(GPTNeoXForCausalLM, self).__init__()
    # Config
    self.config = config
    ...
    self.layers = list(
      [GPTNeoXDecoderLayer(config, layer_idx, parent=self) for layer_idx in range(config.num_hidden_layers)]
    )
...
class GPTNeoXDecoderLayer(NodeMixin):
  # Constructor
  def __init__(self, config: GPTNeoXConfig, layer_idx: int, name=None, parent=None):
    # Config
    self.config = config
