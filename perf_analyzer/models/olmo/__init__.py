...
class OLMoForCausalLM(NodeMixin):
  # Constructor
  def __init__(self, config: OLMoConfig):
    super(OLMoForCausalLM, self).__init__()
    # Config
    self.config = config
    ...
    self.layers = list(
      [OLMoDecoderLayer(config, layer_idx, parent=self) for layer_idx in range(config.num_hidden_layers)]
    )
...
class OLMoDecoderLayer(NodeMixin):
  # Constructor
  def __init__(self, config: OLMoConfig, layer_idx: int, name=None, parent=None):
    super(OLMoDecoderLayer, self).__init__()
    # Config
    self.config = config
