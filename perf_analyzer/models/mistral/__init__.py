...
class MistralForCausalLM(NodeMixin):
  # Constructor
  def __init__(self, config: MistralConfig):
    ...
    self.layers = list(
      [MistralDecoderLayer(config, layer_idx, parent=self) for layer_idx in range(config.num_hidden_layers)]
    )
    self.ln_f = RMSNorm(config.hidden_size, parent=self)
...
class MistralDecoderLayer(NodeMixin):
  # Constructor
  def __init__(self, config: MistralConfig, layer_idx: int, name=None, parent=None):
    ...
    self.parent = parent
    # Construct model
    self.ln_1 = RMSNorm(config.hidden_size, parent=self)
    self.attn = Attention(
      config.hidden_size, 
    ...
    self.ln_2 = RMSNorm(config.hidden_size, parent=self)
    self.mlp = MLP(
      config.hidden_size,
