...
class GPT2ForCausalLM(NodeMixin):
  # Constructor
  def __init__(self, config: GPT2Config):
    super(GPT2ForCausalLM, self).__init__()
    # Config
    self.config = config
    ...
    self.layers = list(
      [GPT2DecoderLayer(config, layer_idx, parent=self) for layer_idx in range(config.num_hidden_layers)]
    )
...
class GPT2DecoderLayer(NodeMixin):
  # Constructor
  def __init__(self, config: GPT2Config, layer_idx: int, name=None, parent=None):
